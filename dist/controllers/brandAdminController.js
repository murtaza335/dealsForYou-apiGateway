...
export const getMyBrand = async (req, res, next) => {
    try {
        const user = await requireRole(req, ["BRAND_ADMIN"]);
        if (!user.brandId)
            return res.status(404).json({ success: false, message: "No brand linked." });
...
export const createMyDeal = async (req, res, next) => {
    try {
        const user = await requireRole(req, ["BRAND_ADMIN"]);
        if (!user.brandId)
            return res.status(404).json({ success: false, message: "No brand linked." });
...
export const deleteMyDeal = async (req, res, next) => {
    try {
        const user = await requireRole(req, ["BRAND_ADMIN"]);
        if (!user.brandId)
            return res.status(404).json({ success: false, message: "No brand linked." });
...
export const listPendingBrands = async (req, res, next) => {
    try {
...
export const rejectBrand = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
...
const attachBrands = async (users) => {
...
export const listBrandAdmins = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
...
export const listEndUsers = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
...
export const suspendBrandAdmin = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
        ...
        if (user.role !== "BRAND_ADMIN")
            throw httpError("User is not a brand admin.", 400);
...
export const deleteBrandAdmin = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
        const userId = String(req.params.userId);
        const user = await userDomainService.getUser(userId);
...
export const deleteEndUser = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
        const userId = String(req.params.userId);
        const user = await userDomainService.getUser(userId);
        ...
        if (user.role !== "END_USER")
            throw httpError("User is not an end user.", 400);
        const clerkUser = await clerkAdminService.deleteUser(user.clerkUserId);
        const deletedUser = await userDomainService.deleteUser(userId);
...
export const getAppAdminOverview = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
...
export const listAllBrandsForAdmin = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
        const brands = await brandAdminService.listBrands();
        res.status(200).json({ success: true, data: brands });
...
export const getBrandForAdmin = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
...
export const getBrandDealsForAdmin = async (req, res, next) => {
    try {
        await requireRole(req, ["APP_ADMIN"]);
