...
const REDIS_FATAL_DISABLE_MS = 60000;
...
function isTlsConfigError(err) {
    ...
    if (["ERR_SSL_PACKET_LENGTH_TOO_LONG", "EPROTO"].includes(code))
        return true;
...
export async function getRedisClient() {
    ...
    if (!client) {
        ...
        newClient.on("error", (err) => {
            if (isTlsConfigError(err)) {
                ...
                console.error(`[Redis] TLS/config error for ${redisTargetLabel(config.url)}; check REDIS_URL scheme/protocol and endpoint`);
                return;
            }
    ...
    connecting = activeClient
        ...
        if (isTlsConfigError(err)) {
            redisDisabledUntil = Date.now() + REDIS_FATAL_DISABLE_MS;
            console.error(`[Redis] connect failed (TLS/config) for ${redisTargetLabel(config.url)}; check REDIS_URL`);
        }
